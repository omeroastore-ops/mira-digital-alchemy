...
export function useConsent() {
  ...
  useEffect(() => {
    try {
      ...
      // Ignore localStorage access errors (e.g. in private browsing modes)
    }
  }, []);
    ...
    <div className="cookie-banner-wrap">
      <div className="cookie-banner-content">
        <div className="text-sm leading-relaxed text-dim max-w-2xl">
          <p className="font-display uppercase tracking-[0.3em] text-[0.65rem] text-bronze mb-2">
            Cookies & Privatsphäre
          </p>
          Wir verwenden Cookies, um Ihr Erlebnis zu verfeinern. Google Maps wird erst geladen, wenn
          Sie zustimmen.{" "}
          <Link
        ...
        <div className="flex gap-3 flex-shrink-0">
          <button onClick={() => setConsent("essential")} className="cookie-banner-btn-secondary">
            Nur Essentielle
          </button>
          <button onClick={() => setConsent("accepted")} className="cookie-banner-btn-primary">
            Alle Akzeptieren
          </button>
        </div>
      </div>
