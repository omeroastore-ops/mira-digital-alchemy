              ...
              <ScrollReveal stagger={true} delay={0.1}>
                <ScrollRevealItem>
                  <div className={`flex items-baseline gap-6 mb-10 ${idx % 2 ? "md-flex-row-reverse md-text-right" : ""}`}>
                    ...
                    <div className="hidden md-block flex-1 h-px" style={{ background: "linear-gradient(to right, rgba(184,137,90,0.4), transparent)" }} />
                  </div>
                </ScrollRevealItem>
                    ...
                    {cat.items.map((item, i) => (
                      <li key={i} className="flex items-baseline group">
                        <span className="font-serif text-lg md-text-xl text-cream group-hover-bronze-bright transition relative menu-item-name inline-block">
                          {item.name}
                          {item.size && <span className="text-xs tracking-wider ml-3 text-dim font-sans">{item.size}</span>}
                        </span>
